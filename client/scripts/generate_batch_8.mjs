...
async function renderBannerForProduct(product) {
  ...
  const brandName = (product.brand?.name || product.brand || 'Liora Exclusive').toString().toUpperCase();
  const categoryName = (product.category?.name || product.category || 'Beauty & Personal Care').toString().toUpperCase();
  ...
  if (product.image) {
    try {
      ...
      // Product hero: target 780x780 bounding box
      ...
      posY = Math.round((950 - meta.height) / 2) + 20; // centered in upper 950px
      resizedProd = fitted;
    } catch (err) {
  ...
  <svg width="1080" height="1080" viewBox="0 0 1080 1080" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- 100% Authentic Guaranteed Stamp -->
    ...
    <!-- Product Title Banner (Floating Top) -->
    ...
    <!-- Discount Ribbon (Top Right Floating) -->
    ${discountPercent > 0 ? `
    ...
    <!-- Bottom High-Impact Price & COD Card -->
    <g transform="translate(60, 890)" filter="url(#heroShadow)">
      <!-- Main Price Banner Bar -->
...
async function main() {
  ...
  console.log(`Found ${nextProds.length} new in-stock products for Batch 8.`);

  // Write directly into client/public/banners and client/scratch/banners
